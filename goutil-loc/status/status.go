...
func (s *Status) QueryString() string {
	return goutil_loc.BytesToString(s.EncodeQuery())
}
...
func (s *Status) JSONString() string {
	...
	b, _ := s.MarshalJSON()
	return goutil_loc.BytesToString(b)
}
...
func (s *Status) MarshalJSON() ([]byte, error) {
	...
	b = append(b, reB...)
	b = append(b, goutil_loc.StringMarshalJSON(s.msg, false)...)
	...
	b = append(b, reC...)
	b = append(b, goutil_loc.StringMarshalJSON(cause, false)...)
...
func (s *Status) EncodeQuery() []byte {
	...
	if len(s.msg) > 0 {
		...
		b = append(b, keyMsg...)
		b = append(b, '=')
		b = appendQuotedArg(b, goutil_loc.StringToBytes(s.msg))
	}
	if s.cause != nil {
		b = append(b, '&')
		b = append(b, keyCause...)
		b = append(b, '=')
		b = appendQuotedArg(b, goutil_loc.StringToBytes(s.cause.Error()))
	}
	return b
}
...
func (s *Status) DecodeQuery(b []byte) {
	...
	for a.next(kv) {
		if !hadCode && bytes.Equal(keyCode, kv.key) {
			i, _ := strconv.ParseInt(goutil_loc.BytesToString(kv.value), 10, 32)
			s.code = int32(i)
			if hadMsg && hadCause {
				return
			}
			...
			if hadCode && hadCause {
				s.msg = goutil_loc.BytesToString(kv.value)
				return
			}
			...
			if hadCode && hadMsg {
				s.cause = errors.New(goutil_loc.BytesToString(kv.value))
				return
			}
